...
__device__ __forceinline__ void load_A(int8_t* dstA, const int8_t* A_all, size_t t, int N, int row0){
  ...
  for(int i=tid;i<BMG*nch;i+=NTHREADS){ int r=i/nch,c=i%nch; cpa16(&dstA[r*KC+c*16],&base[(size_t)r*KC+c*16]); }
}
    ...
    int N, int T, int reverse, int store_all)
{
  ...
  int8_t* sA = smem;                    // [STAGES_A][BMG][KC]
  float* sSc = (float*)(sA + (size_t)STAGES_A*BMG*KC);  // [8][HX] resident scales/biases
  ...
  for(int t=0;t<T;++t){
...
#if PIPE
    if(t+1<T){ int nxt=(t+1)&(STAGES_A-1); load_A(sA+(size_t)nxt*BMG*KC,A_all,tstep(t+1),N,row0);
      asm volatile("cp.async.commit_group;\n"); }
    ...
    for(int m=0;m<MSET;++m){
      ...
      for(int kt=0;kt<8;++kt){
        ...
        uint32_t a=smem_addr(&Acur[(mrow0+r)*KC + kt*32 + koff]);
        ldm_x4(Af[m][kt][0],Af[m][kt][1],Af[m][kt][2],Af[m][kt][3],a);
      }
#else
      #pragma unroll
      for(int kt=0;kt<8;++kt){ int co=kt*32+tg*4;
        Af[m][kt][0]=*(const int*)&Acur[(mrow0+gid)*KC+co];    Af[m][kt][1]=*(const int*)&Acur[(mrow0+gid+8)*KC+co];
        Af[m][kt][2]=*(const int*)&Acur[(mrow0+gid)*KC+co+16]; Af[m][kt][3]=*(const int*)&Acur[(mrow0+gid+8)*KC+co+16]; }
#endif
    }
    ...
    for(int nn=0;nn<HX/8;++nn){
      ...
      // EPI HOIST: scales/biases depend on channel lcol, NOT row m -> re-read MSET x redundantly.
      // lcol = nn*8+2*tg+(e&1) -> only 2 distinct channels per (nn,tg). Load their 8 scales ONCE.
      int l0=nn*8+2*tg;
      float sc0[8], sc1[8];
      #pragma unroll
      for(int g=0;g<8;++g){ sc0[g]=sSc[g*HX+l0]; sc1[g]=sSc[g*HX+l0+1]; }
      #pragma unroll
      for(int m=0;m<MSET;++m){
        int mrow0 = wr0 + m*16;
        #pragma unroll
        for(int e=0;e<4;++e){
          ...
          int8_t hn=epi_elem(cg[m][0][e],cg[m][1][e],cg[m][2][e],cg[m][3][e],
              sc[0],sc[1],sc[2],sc[3],sc[4],sc[5],sc[6],sc[7],AS,cv);
#if CELLREG
...
int main(int argc,char**argv){
  ...
  size_t smem=(size_t)STAGES_A*BMG*KC + (size_t)8*HX*4;
  int ncta=GX*(N/BMG);
