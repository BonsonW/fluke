    ...
    int N, int T, int reverse, int store_all)
{
  ...
  for(int t=0;t<T;++t){
    ...
    for(int nn=0;nn<HX/8;++nn){
      ...
      // REGISTER-BLOCKED PIPELINE: load ALL 4 gates' weight frags up front (lets the __ldg
      // overlap), then kt-OUTER MMA -> per kt, MSET*4=8 INDEPENDENT accumulator chains issued
      // back-to-back => max MMA ILP to hide the pipeline `wait` latency at 8 warps/1-CTA-SM.
      int B0[4][8], B1[4][8];
      #pragma unroll
      for(int g=0;g<4;++g){
        ...
        B0[g][0]=w0.x;B0[g][1]=w0.z;B0[g][2]=w1.x;B0[g][3]=w1.z;B0[g][4]=w2.x;B0[g][5]=w2.z;B0[g][6]=w3.x;B0[g][7]=w3.z;
        B1[g][0]=w0.y;B1[g][1]=w0.w;B1[g][2]=w1.y;B1[g][3]=w1.w;B1[g][4]=w2.y;B1[g][5]=w2.w;B1[g][6]=w3.y;B1[g][7]=w3.w;
      }
      #pragma unroll
      for(int kt=0;kt<8;++kt)
        #pragma unroll
        for(int g=0;g<4;++g)
          ...
          for(int m=0;m<MSET;++m)
            mma_m16n8k32(cg[m][g][0],cg[m][g][1],cg[m][g][2],cg[m][g][3],
                         Af[m][kt][0],Af[m][kt][1],Af[m][kt][2],Af[m][kt][3],B0[g][kt],B1[g][kt]);
      #pragma unroll
      for(int m=0;m<MSET;++m){
        int mrow0 = wr0 + m*16;
        #pragma unroll
