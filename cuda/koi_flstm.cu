...
// ================= GATE CTA (gx 0..7) =================
// resident gate-weight slice sWg [4*HXG*Kc] (128KB, repacked). A staged [BM,Kc].
__device__ void gate_cta(int8_t* smem, const int8_t* wg_rp, const float* wscale,
  ...
  int8_t* sWg = smem;                // resident [4*HXG*Kc]=128KB
  int8_t* sA  = sWg + 4*HXG*KC;      // [BM, Kc]
  for(int i=tid;i<4*HXG*KC;i+=NTHREADS){
    int g=i/(HXG*KC), rem=i%(HXG*KC), col=rem/KC, kc=rem%KC;
  ...
  for(int t=0;t<T;++t){
    ...
    for(int j=tid;j<BM*(K_HH/16);j+=NTHREADS){ int r=j/(K_HH/16), c=j%(K_HH/16);
      ((int4*)sA)[r*(KC/16)+c]=*(const int4*)&sc[(long)(row0+r)*K_HH+c*16]; }
    for(int j=tid;j<BM*(R/16);j+=NTHREADS){ int r=j/(R/16), c=j%(R/16);
      ((int4*)sA)[r*(KC/16)+(K_HH/16)+c]=*(const int4*)&xt[(long)(row0+r)*R+c*16]; }
    __syncthreads();
    // M-split gate: warp=16 rows; loop HXG/8 n-tiles; kt-outer 4-gate ILP; resident W.
    for(int rt=0; rt<BM/16; ++rt){
      if((rt&7)!=warp) continue;
      int A[8][4];
      ...
      for(int nn=0;nn<HXG/8;++nn){
        int B0[4][8], B1[4][8];
        for(int g=0;g<4;++g){
          const int8_t* wgs=sWg + g*HXG*KC + (nn*8+gid)*KC + tg*64;
          int4 w0=*(const int4*)(wgs), w1=*(const int4*)(wgs+16), w2=*(const int4*)(wgs+32), w3=*(const int4*)(wgs+48);
          B0[g][0]=w0.x;B1[g][0]=w0.y;B0[g][1]=w0.z;B1[g][1]=w0.w; B0[g][2]=w1.x;B1[g][2]=w1.y;B0[g][3]=w1.z;B1[g][3]=w1.w;
          B0[g][4]=w2.x;B1[g][4]=w2.y;B0[g][5]=w2.z;B1[g][5]=w2.w; B0[g][6]=w3.x;B1[g][6]=w3.y;B0[g][7]=w3.z;B1[g][7]=w3.w;
        }
        int cg[4][4];
        for(int g=0;g<4;++g) for(int e=0;e<4;++e) cg[g][e]=0;
        for(int kt=0;kt<8;++kt)               // kt-outer -> 4 independent gate mma chains
          for(int g=0;g<4;++g)
            mma_m16n8k32(cg[g][0],cg[g][1],cg[g][2],cg[g][3],A[kt][0],A[kt][1],A[kt][2],A[kt][3],B0[g][kt],B1[g][kt]);
        for(int e=0;e<4;++e){
...
int main(int argc,char**argv){
  ...
  size_t smem_g=(size_t)4*HXG*KC + (size_t)BM*KC;   // gate: 128KB + 32KB
