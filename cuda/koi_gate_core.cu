    ...
    int N, int T, int reverse, int store_all)
{
  ...
  int core_sink=0;   // Phase-0 probe sink: keep work live. CORE=MMA-only; NOWRITE=+epi/cell, -writeout
#endif
  ...
  for(int t=0;t<T;++t){
    ...
    for(int nn=0;nn<HX/8;++nn){
      ...
      for(int m=0;m<MSET;++m)
        ...
        for(int g=0;g<4;++g)
          #pragma unroll
          for(int e=0;e<4;++e) core_sink += cg[m][g][e];   // keep all MMAs live
    }
#else
      #pragma unroll
      for(int m=0;m<MSET;++m){
...
#endif
#ifdef NOWRITE
          core_sink += hn;                    // sink hn: keep epi/cell live, skip writeout
#else
