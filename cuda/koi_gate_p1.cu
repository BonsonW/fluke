...
// ==================================================================================
//
// koi_gate_w16.cu -- koi_gate_sm + 16-BIT-PACKED staging stores (BEST gate: 18.0us @N1536).
// ==================================================================================
// FINDING (2026-07-09, Phase 2a): the coalesced writeout staged h via 64 scattered STS.U8/step
// (sub-word smem byte-stores -- expensive). Each thread's e0,e1 land in ADJACENT cols of one row
// (and e2,e3 in the gid+8 row), so pack each pair into one STS.U16 -> 32 aligned 16-bit stores.
// RESULT: 22.6 -> 18.0us/step @N1536 (~20%), BIT-EXACT fwd+reverse. w16 (17.98) == NOWRITE probe
// (18.16) => the writeout is now essentially FREE; the byte-store was the whole ~4.4us writeout
// cost. Decomposition of the remaining 18us: MMA compute 14us (37% imma, wait-bound) + epi math
// 4us. Next targets: MMA-ILP (Phase 1) and epi-math. Ship-candidate base.
// ==================================================================================
//
    ...
    int N, int T, int reverse, int store_all)
{
  ...
  __half creg[HX/8][MSET][4];   // f16-resident cell (bit-identical: value already f16-rounded), frees regs
  #pragma unroll
  for(int a=0;a<HX/8;++a) for(int b=0;b<MSET;++b) for(int c=0;c<4;++c) creg[a][b][c]=(__half)0.f;
#endif
  ...
  for(int t=0;t<T;++t){
    ...
    #pragma unroll 1
#endif
    for(int nn=0;nn<HX/8;nn+=2){          // PHASE 1: interleave 2 nn-tiles -> 16 independent accumulators
      int cg[2][MSET][4][4];
      #pragma unroll
      for(int q=0;q<2;++q) for(int m=0;m<MSET;++m) for(int g=0;g<4;++g) for(int e=0;e<4;++e) cg[q][m][g][e]=0;
      #pragma unroll
      for(int g=0;g<4;++g){
        int b0[2][8], b1[2][8];
        #pragma unroll
        for(int q=0;q<2;++q){
          const int8_t* wgs = wg_rp + (long)g*H*KC + (long)(hbase+(nn+q)*8+gid)*KC + tg*64;
          int4 w0=__ldg((const int4*)(wgs)),  w1=__ldg((const int4*)(wgs+16)),
               w2=__ldg((const int4*)(wgs+32)),w3=__ldg((const int4*)(wgs+48));
          b0[q][0]=w0.x;b0[q][1]=w0.z;b0[q][2]=w1.x;b0[q][3]=w1.z;b0[q][4]=w2.x;b0[q][5]=w2.z;b0[q][6]=w3.x;b0[q][7]=w3.z;
          b1[q][0]=w0.y;b1[q][1]=w0.w;b1[q][2]=w1.y;b1[q][3]=w1.w;b1[q][4]=w2.y;b1[q][5]=w2.w;b1[q][6]=w3.y;b1[q][7]=w3.w;
        }
        #pragma unroll
        for(int m=0;m<MSET;++m)
          #pragma unroll
          for(int kt=0;kt<8;++kt)
            #pragma unroll
            for(int q=0;q<2;++q)          // 2 independent accumulator chains interleaved per (m,kt)
              mma_m16n8k32(cg[q][m][g][0],cg[q][m][g][1],cg[q][m][g][2],cg[q][m][g][3],
                           Af[m][kt][0],Af[m][kt][1],Af[m][kt][2],Af[m][kt][3],b0[q][kt],b1[q][kt]);
      }
      #pragma unroll
      for(int q=0;q<2;++q){
        int nnl=nn+q;
        ...
        for(int m=0;m<MSET;++m){
          ...
          #pragma unroll
          for(int e=0;e<4;++e){
            int lcol=nnl*8+2*tg+(e&1), rin=(e<2)?gid:gid+8;
            int oc=hbase+lcol, gr=row0+mrow0+rin;
#if CELLREG
            float cv=(float)creg[nnl][m][e];
#else
            float cv=(float)cell[(long)gr*H+oc];
#endif
            hh[e]=epi_elem(cg[q][m][0][e],cg[q][m][1][e],cg[q][m][2][e],cg[q][m][3][e],
                sSc[0*HX+lcol],sSc[1*HX+lcol],sSc[2*HX+lcol],sSc[3*HX+lcol],
                sSc[4*HX+lcol],sSc[5*HX+lcol],sSc[6*HX+lcol],sSc[7*HX+lcol],1.0f,cv);
#if CELLREG
            creg[nnl][m][e]=(__half)cv;    // f16 round (bit-identical to prior (float)(cellT)cv)
#else
