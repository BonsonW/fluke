...
// epilogue variant with PRE-SCALED gate scales (asi=as*si, hoisted out of the m_it loop):
// numerically identical to epi_elem (same fmul then fmaf) -> BIT-EXACT vs the ref.
#ifndef F16TANH
#define F16TANH 0
#endif
__device__ __forceinline__ int8_t epi_elem2(int gi,int gf,int gg,int go,
    float asi,float asf,float asg,float aso,float bi,float bf,float bg,float bo,float&cell){
  float vi=__fmaf_rn((float)gi,asi,bi), vf=__fmaf_rn((float)gf,asf,bf);
  float vg=__fmaf_rn((float)gg,asg,bg), vo=__fmaf_rn((float)go,aso,bo);
  float I=fminf(fmaxf(__fmaf_rn(vi,0.2f,0.5f),0.f),1.f);
  float F=fminf(fmaxf(__fmaf_rn(vf,0.2f,0.5f),0.f),1.f);
  float O=fminf(fmaxf(__fmaf_rn(vo,0.2f,0.5f),0.f),1.f);
  float G=fminf(fmaxf(vg,-1.f),1.f);
  cell=__fmaf_rn(F,cell,I*G);
#if F16TANH
  __half ch=__float2half(cell); unsigned short hb=*(unsigned short*)&ch;
  asm("tanh.approx.f16 %0, %1;\n":"=h"(hb):"h"(hb)); ch=*(__half*)&hb;
  return clamp_i8(rintf(O*__half2float(ch)*127.0f));
#else
  return clamp_i8(rintf(O*tanhf(cell)*127.0f));
#endif
}
    ...
    int Nrows)
{
  ...
  // fused LSTM epilogue (OPT: channel-outer -> the 8 scale/bias smem reads + the as-multiply
  // are m_it-INVARIANT (cloc doesn't depend on m_it), so hoist them out of the IterM loop:
  // 8 sSc reads + 4 muls done ONCE per channel instead of IterM=4 times).
  #pragma unroll
  for(int b=0;b<NB32;++b)
    #pragma unroll
    for(int e=0;e<4;++e){
      int cloc = (wn*WpShape::kN + b*32)/4 + 2*tg + (e&1);   // channel within window
      float asi=AS*sSc[0*CH_TB+cloc], asf=AS*sSc[1*CH_TB+cloc], asg=AS*sSc[2*CH_TB+cloc], aso=AS*sSc[3*CH_TB+cloc];
      float bbi=sSc[4*CH_TB+cloc], bbf=sSc[5*CH_TB+cloc], bbg=sSc[6*CH_TB+cloc], bbo=sSc[7*CH_TB+cloc];
      #pragma unroll
      for(int m_it=0;m_it<IterM;++m_it){
        ...
        int8_t hn = epi_elem2(gi,gf,gg,go, asi,asf,asg,aso, bbi,bbf,bbg,bbo, cv);
