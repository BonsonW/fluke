...
// ── Factored-LSTM (CRF/LSTM model): int8 down-projection + gate step ───────────
...
// ── Unified recurrence: fluke owns the T-step loop + CUDA graph ───────────────────────────────────
...
fluke_flstm_rec_t *fluke_flstm_rec_create(const fluke_flstm_backend_t *b, int N, int T, int num_layers) {
    ...
    r->hh_down = r->a_scratch = NULL; r->ones = NULL;
...
int fluke_flstm_recurrence(
    ...
    auto step = [&](int i) {
        ...
        // (per-channel scale), then concat [hh_down | x] via D2D, then the gate step.
