...
_HERE = os.path.dirname(os.path.abspath(__file__))          # fly/rdna4/dual_gemm_silu
_RDNA4 = os.path.dirname(_HERE)                             # fly/rdna4
sys.path.insert(0, _HERE)
sys.path.insert(0, _RDNA4)                                 # shared _fp8_export
sys.path.insert(0, os.path.join(_RDNA4, "quantize"))       # rdna_fp8_per_token_quantize
...
def _compile_and_get_ir_gemm(M: int, N: int, K: int) -> str:
    ...
    try:
        ...
        pass  # non-local arch launch fails after compile; we only need the compiled module
...
def _export_gemm(M: int, N: int, K: int, artifacts_dir: str) -> None:
    ...
    header += fe.module_loaddata_fn("fp8_dual_silu", kernel_name)
...
def _export_ptq(M: int, N: int, artifacts_dir: str) -> None:
    ...
    header += fe.module_loaddata_fn("fp8_ptq", kernel_name)
    fe.write_artifacts(artifacts_dir, name, hsaco, header)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--arch", action="append",
                        help="RDNA4 arch(es) to export (default: all of %s)" % fe.RDNA4_ARCHS)
    args = parser.parse_args()

    configs = (_load_configs(args.config, default_m=args.M) if args.config
               else [(args.M, args.N, args.K)])

    for arch in (args.arch or fe.RDNA4_ARCHS):
        print(f"\n=== arch {arch} ===")
        fe.set_compile_arch(arch)
        compile_fp8_dual_gemm_silu.cache_clear()
        compile_fp8_per_token_quantize.cache_clear()
        artifacts_dir = fe.artifacts_dir_for(arch)
        for M, N, K in configs:
            _export_one(M, N, K, artifacts_dir)
        print(f"Done: {len(configs)} config(s) -> {artifacts_dir}/")
