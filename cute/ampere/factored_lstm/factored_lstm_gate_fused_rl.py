"""REGISTER-LOCAL fused-interleaved INT8 gate factored-LSTM step (Ampere/A100, sm80).

Fastest CuTe-DSL port of the CUTLASS-C++ prototype (fluke/cuda/fused_cutlass.cu,
30.2us gate / 37us step at N=2048).  This is the DSL sibling of
factored_lstm_gate_fused_i8.py; the difference is the EPILOGUE.

  * ONE plain GEMM: gate weights are INTERLEAVED host-side in 8-column blocks --
    GEMM column n = B32*32 + g*8 + p -> gate g of hidden channel B32*8 + p.
  * KEY: atom_layout_N == 1 with a contiguous 32-wide warp N-tile
    (permutation_mnk[1]=32) so all 4 gate n8-tiles of a channel land in ONE thread's
    C-fragment (n%4 = gate).  This makes the LSTM update fully REGISTER-LOCAL -- the
    C++ trick that fused_gate_i8 (atom_N=2) could NOT use: there g1/g3 live in a
    different WARP's fragment, forcing a mandatory int32 acc smem round-trip.
  * Epilogue: read i/f/g/o straight from registers, dequant + sighard/tanh cell
    update, stage h(int8)+cell(f32) to smem, then TWO coalesced tile writeouts
    (h packed 4->int32).  NO acc smem staging, ONE syncthreads.  The [B,4H] int32 C
    is never materialized; the cell tile is cp.async-prefetched during the mainloop.

  raw_g = acc_g * scale_a[m] * scale_w[g][n] + bias_g
  i,f,o = clamp(0.2*raw+0.5,0,1); g = clamp(raw,-1,1)
  c'    = f*c + i*g ;  h = o*tanh(c') -> int8 @ 1/127

RESULTS (A100, N=2048, warm, median of 7 x 1000, test_factored_lstm_gate_fused_rl.py):
  bit-exact vs int8-recompute ref (h max_abs 0.0039 = 1 quantum; cell ~exact).
  *** WINNER bm=128 bn=64 bk=64 st=3 atom=(4,1,1): 33.4us *** = ~1.11x the C++ gate
  (30.2us) -> DSL is at ~parity.  vs the smem-staged sibling fused_gate_i8 (best
  43us after coalesced-acc-store + smem-alias + packed writeout) and the original
  scalar-epilogue prototype (48us).  Mainloop-only floor (gutted epilogue) is 27us
  (already BELOW the C++ gate) -> the whole residual was the epilogue; going
  register-local recovered ~10us of it.  Configs that spill (bn>=128 & atom=(2,1,1),
  bn=256) blow up to 100s of us -- keep the fragment small (bm=128 bn=64 => 32
  int32 accum/thread).  NOTE mC_c must be a STATIC layout (from_dlpack(assumed_align
  =16), no mark_layout_dynamic) or the cell-prefetch cp.async fails its align proof.
"""
...
class TensorOpFactoredLstmGateFusedI8(TensorOpGemmI8):
    """One interleaved int8 gate GEMM (pipelined) + REGISTER-LOCAL fused LSTM epilogue.
    Requires atom_layout_mnk = (M, 1, 1) (contiguous 32-wide warp N-tile). Winner:
    bm=128 bn=64 bk=64 st=3 atom=(4,1,1) -> 33.4us @ N=2048 (A100)."""
    ...
    def __init__(
        ...
        assert atom_lay_K == 1
        assert atom_lay_N == 1, "register-local epilogue needs a contiguous 32-wide warp N tile"
    ...
    def __call__(
        ...
        # REGISTER-LOCAL epilogue: the 4 gates for a channel live in ONE thread's fragment
        # (contiguous 32-wide warp N-tile), so NO int32 acc smem staging is needed at all.
        ...
        permutation_mnk = (
            ...
            32,   # contiguous 4-gate (i/f/g/o) block per warp -> gates register-local
        ...
        _launcher = self.kernel(
            ...
            mC_c, mH_out,
            sA_layout, sB_layout, sCell_layout, sSa_layout, sH_layout,
    ...
    def kernel(
        ...
        sCell_layout: cute.Layout, sSa_layout: cute.Layout,
        ...
        if grid_dim[0] <= offset_tile_x or grid_dim[1] <= offset_tile_y:
            ...
            # The 4 gate accumulators of a channel live in ONE thread's C-fragment
            # (contiguous 32-wide warp N-tile -> n%4 = gate). No acc smem exchange: read
            # i/f/g/o straight from registers, LSTM-update, stage h+cell to smem, then
            # do the two coalesced tile writeouts.
            def clamp(x, lo, hi):
            ...
            num_blk = cutlass.const_expr(num_mma_n // 4)   # 32-col gate blocks per warp
            for i in cutlass.range(num_vals, unroll_full=True):
                for m in cutlass.range(num_mma_m, unroll_full=True):
                    for b in cutlass.range(num_blk, unroll_full=True):
                        coord = tCcC[i, m, b * 4]           # gate-0 element's (row,col)
                        rloc = coord[0]
                        col0 = coord[1]
                        chloc = (col0 // 32) * 8 + (col0 % 8)
                        gch = ch0 + chloc
                        acc_i = tCrC[i, m, b * 4 + 0].to(cutlass.Float32)
                        acc_f = tCrC[i, m, b * 4 + 1].to(cutlass.Float32)
                        acc_g = tCrC[i, m, b * 4 + 2].to(cutlass.Float32)
                        acc_o = tCrC[i, m, b * 4 + 3].to(cutlass.Float32)
                        sa = sSa[rloc].to(cutlass.Float32)
                        ...
                        c_new = f_a * sCell[rloc, chloc].to(cutlass.Float32) + i_a * g_a
                        sCell[rloc, chloc] = c_new           # overwrite -> coalesced writeout
                        h_new = o_a * cute.math.tanh(c_new)
                        hs = h_new * 127.0
                        half = clamp(hs * 1e30, -0.5, 0.5)
                        sH[rloc, chloc] = (hs + half).to(self.out_dtype)
            cute.arch.sync_threads()

            # coalesced tile writeout: int8 h packed to int32, and f32 cell.
            ...
            ncell = cutlass.const_expr(self.bM * CH_TB)
            citer = cutlass.const_expr((ncell + nt - 1) // nt)
            for j in cutlass.range_constexpr(citer):
                w = tidx + j * nt
                if w < ncell:
                    r = w // CH_TB
                    c = w % CH_TB
                    mC_flat[(row0 + r) * Hn + ch0 + c] = sCell[r, c]
        return
